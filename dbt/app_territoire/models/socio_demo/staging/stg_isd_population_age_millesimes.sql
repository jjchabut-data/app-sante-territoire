...
-- Chaque source sélectionne ses colonnes explicitement par nom (pas `select *`)
-- avant l'UNION ALL : l'ordre physique des colonnes diffère entre les tables
-- raw (créées à des moments différents, schéma auto-détecté par BigQuery),
-- et `select *` unirait les colonnes par position plutôt que par nom.
...
with source_2023 as (
    select
        ...
        `annee`                as annee
    from {{ source('raw', 'isd_c01_pop_selon_age_2023') }}
),
...
source_2024 as (
    select
        ...
        `% 85 ans ou plus`     as pct_85_plus,
        `annee`                as annee
    from {{ source('raw', 'isd_c01_pop_selon_age_2024') }}
),
...
source_2025 as (
    select
        ...
        `% 85 ans ou plus`     as pct_85_plus,
        `annee`                as annee
    from {{ source('raw', 'isd_c01_pop_selon_age_2025') }}
),
...
source_2026 as (
    select
        ...
        `% 85 ans ou plus`     as pct_85_plus,
        `annee`                as annee
    from {{ source('raw', 'isd_c01_pop_selon_age_2026') }}
),
