...
def load_ref_territoires():
    return pd.read_parquet(f'{DATA_DIR}/ref/ref_app_territoires.parquet')

@st.cache_data
def load_geom_communes():
    return gpd.read_parquet(f'{GEOM_DIR}/app_communes_geom_simplified.parquet')

@st.cache_data
def load_geom_dept():
    return gpd.read_parquet(f'{GEOM_DIR}/departements_geom_simplified.parquet')

@st.cache_data
def load_geom_epci():
    return gpd.read_parquet(f'{GEOM_DIR}/epci_geom_simplified.parquet')

@st.cache_data
def load_geom_regions():
    return gpd.read_parquet(f'{GEOM_DIR}/regions_geom_simplified.parquet')

@st.cache_data
def load_geom_ze():
    return gpd.read_parquet(f'{GEOM_DIR}/zone_emploi_geom.parquet')

@st.cache_data
def load_clusters():
    """Charge les clusters pré-calculés (K-Means k=5, trié par score APL composite)."""
    return pd.read_parquet(f'{DATA_DIR}/data/commune/commune_clusters.parquet')

@st.cache_data
def load_ze_communes_mapping():
    df = pd.read_parquet('../data/raw/geo/zone_emploi_communes.parquet')
    return dict(zip(df['CODGEO'], df['ZE2020']))

@st.cache_data
def load_dept_centroids():
    """Centroides des départements pour le TextLayer."""
    gdf = gpd.read_parquet(f'{GEOM_DIR}/departements_geom_simplified.parquet')
...
def load_geom_iris(codes: tuple):
    ...
    return gpd.read_parquet(
        '../data/processed/ref/ref_iris_habitat_geom_simplified.parquet',
...
def load_fdep():
    ...
    return pd.read_parquet('../data/processed/social/app_fdep_utile.parquet')
